...
def Benchmarking_CNN(dataset,filename, input_size, optimizer):
    ...
    plt.ylabel('Loss')
    plt.xlabel('Epochs')
    plt.savefig(filename+'CNN Loss History with '+optimizer+' Optimiser.png')
steps = 500


filename=r'C:\Users\ivoll\OneDrive\Documents\qml_sin-3 copy\Final_Results_CNN\new_result_CNN_adam_d100000'
for i in range(0,31):
    print('running',i)
    dataset = sin_generator.sin_gen2(i,10000)
